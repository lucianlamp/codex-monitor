# Codex Control Bridge Design Spec

Status: design approved by the active goal contract; no implementation code exists in this
repository yet.
...
Build `codex-control-bridge`, a local-first bridge that delivers external
...
- Repository and directory: `codex-control-bridge`
- Cargo package name: `codex-control-bridge`
- Primary binary: `codex-control-bridge`
- Alias binary: `ccb`
- Library crate import name: `codex_control_bridge`
- Codex app-server `clientInfo.name`: `codex-control-bridge`
- Codex app-server `clientInfo.title`: `Codex Control Bridge`

## Evidence checked before design

- Current directory `/Users/ysk411/dev/codex-control-bridge` was empty and was
...
```text
source adapter(s)
  -> BridgeEvent normalizer
  -> delivery queue and dedupe policy
  -> target resolver
  -> Codex app-server client
  -> transport implementation
  -> Codex App daemon or ccb-managed app-server
```

The crate is a Rust CLI/library project. The package name stays hyphenated
(`codex-control-bridge`), while the Rust library crate is imported as
`codex_control_bridge`.

The implementation should keep protocol construction and transport mechanics in
the library so both binaries call the same code path. `ccb` is an alias binary,
...
- `delivery`: event queueing, dedupe, turn-start formatting, inject-items
  guardrails, and retry rules.
- `sources`: source adapter trait and source event model.
...
For ccb-managed sessions, the bridge picks a free loopback port, launches:
...
- `managed`: ccb starts or attaches to a ccb-owned app-server process, normally
  over loopback WebSocket. This is the cross-platform default.
- `app`: ccb attaches to an existing Codex App managed daemon, normally over
...
- `--target managed`: use a ccb-managed loopback WebSocket app-server.
...
1. Send `initialize` with:
   ...
   {
     "clientInfo": {
       "name": "codex-control-bridge",
       "title": "Codex Control Bridge",
       "version": "0.1.0"
     },
...
`ccb threads --cwd <path>` is read-only. It prints candidate thread ids,
...
transport, terminate any ccb-managed child process, and exit non-zero. In
`agmsg watch`, the source event that triggered the server request is not marked
delivered in ccb state, so a later retry remains possible after human review.
...
- Handles SIGINT/SIGTERM by stopping the source adapter, closing the transport,
  and terminating any ccb-managed app-server child process.
...
```bash
codex app-server --listen ws://127.0.0.1:<port>
ccb --endpoint ws://127.0.0.1:<port> threads --cwd /Users/ysk411/dev/codex-control-bridge
```
...
- Cargo package is named `codex-control-bridge`.
- Binaries `codex-control-bridge` and `ccb` both call the same library CLI.
- `clientInfo.name` is exactly `codex-control-bridge`.
- `ccb threads --cwd <path>` works against a fake app-server and a real
  loopback app-server smoke target.
- `ccb send --thread <id> --text <msg>` builds the correct `turn/start`
  request and handles completion/failure/server-request paths in tests.
- `ccb agmsg watch --team <team> --name <agent> --thread <id>` delivers fixture
