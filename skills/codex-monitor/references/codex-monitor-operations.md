...
The default runs `codex-monitor app-hook enable --team <team> --name <name>
...
continuation is interrupted by a new user input, the hook keeps a session-private
pending delivery and replays it at the next ordinary Stop. It acknowledges and
clears pending content only after the continuation itself reaches Stop, giving
at-least-once delivery instead of silently losing an already-read inbox row. If the
...
matching heartbeat; disabling also clears only that session's pending delivery.
All
